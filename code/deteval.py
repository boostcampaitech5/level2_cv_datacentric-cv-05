...
def calc_deteval_metrics(
    pred_bboxes_dict,
    gt_bboxes_dict,
    transcriptions_dict=None,
    eval_hparams=None,
    bbox_format="rect",
    verbose=False,
):
    ...
    def one_to_one_match(row, col):
        ...
        for j in range(len(recallMat[0])):
            if (
                recallMat[row, j] >= eval_hparams["AREA_RECALL_CONSTRAINT"]
                and precisionMat[row, j] >= eval_hparams["AREA_PRECISION_CONSTRAINT"]
            ):
                cont = cont + 1
        if cont != 1:
            return False
        ...
        for i in range(len(recallMat)):
            if (
                recallMat[i, col] >= eval_hparams["AREA_RECALL_CONSTRAINT"]
                and precisionMat[i, col] >= eval_hparams["AREA_PRECISION_CONSTRAINT"]
            ):
                cont = cont + 1
        if cont != 1:
            return False
        ...
        if (
            recallMat[row, col] >= eval_hparams["AREA_RECALL_CONSTRAINT"]
            and precisionMat[row, col] >= eval_hparams["AREA_PRECISION_CONSTRAINT"]
        ):
            return True
        return False
        ...
        for detNum in range(len(recallMat[0])):
            if (
                ...
                if (
                    ...
                    >= eval_hparams["AREA_PRECISION_CONSTRAINT"]
                ):
    ...
    _pred_bboxes_dict, _gt_bboxes_dict = deepcopy(pred_bboxes_dict), deepcopy(
        gt_bboxes_dict
    )
    ...
    for sample_name in gt_bboxes_dict:
        ...
        if sample_name in pred_bboxes_dict:
            ...
            if len(detRects) > 0:
                ...
                precision_cond = (
                    precisionMat >= eval_hparams["AREA_PRECISION_CONSTRAINT"]
                )
                        ...
                        if (
                            ...
                            if match is True:
                                ...
                                if is_single_overlap(gtNum, detNum) is True:
                                    ...
                                    if (
                                        normDist
                                        < eval_hparams["EV_PARAM_IND_CENTER_DIFF_THR"]
                                    ):
                                        gtRectMat[gtNum] = 1
                                        detRectMat[detNum] = 1
                    ...
                    if gtNum not in gtDontCareRectsNum:
                        ...
                        if match is True:
                            ...
                            if num_overlaps_gt(gtNum) >= 2:
                                gtRectMat[gtNum] = 1
                                recallAccum += (
                                    eval_hparams["MTYPE_OO_O"]
                                    if len(matchesDet) == 1
                                    else eval_hparams["MTYPE_OM_O"]
                                )
                                precisionAccum += (
                                    eval_hparams["MTYPE_OO_O"]
                                    if len(matchesDet) == 1
                                    else eval_hparams["MTYPE_OM_O"] * len(matchesDet)
                                )
                                pairs.append(
                                    {
                                        "gt": gtNum,
                        ...
                        if match is True:
                            ...
                            if num_overlaps_det(detNum) >= 2:
                                ...
                                recallAccum += (
                                    eval_hparams["MTYPE_OO_O"]
                                    if len(matchesGt) == 1
                                    else eval_hparams["MTYPE_OM_M"] * len(matchesGt)
                                )
                                precisionAccum += (
                                    eval_hparams["MTYPE_OO_O"]
                                    if len(matchesGt) == 1
                                    else eval_hparams["MTYPE_OM_M"]
                                )
                                pairs.append(
                                    {
                                        "gt": matchesGt,
...
#     with open(file="/opt/ml/level2_cv_datacentric-cv-05/code/pths/pred.pkl", mode="rb") as f:
#         extra_info = pickle.load(f)
#     with open(file="/opt/ml/level2_cv_datacentric-cv-05/code/pths/geo.pkl", mode="rb") as f:
#         gt_geo = pickle.load(f)
#     with open(file="/opt/ml/level2_cv_datacentric-cv-05/code/pths/score.pkl", mode="rb") as f:
#         gt_score = pickle.load(f)
#     p_geo_map = extra_info['geo_map']
#     p_score_map = extra_info['score_map']
#     pred = {}
#     gt = {}
#     for i in range(len(p_geo_map)):
#         pred_geo_single = p_geo_map[i]
#         pred_score_single = p_score_map[i]
#         gt_geo_single = gt_geo[i]
#         gt_score_single = gt_score[i]

#         pred_poly =get_bboxes(pred_score_single.detach().cpu().numpy(),pred_geo_single.detach().cpu().numpy())
#         gt_poly =get_bboxes(gt_score_single.detach().cpu().numpy(),gt_geo_single.detach().cpu().numpy())
#         pred_rect = pred_poly[:,[0,1,4,5]]
#         gt_rect = gt_poly[:,[0,1,4,5]]
#         pred[str(i)] = pred_rect
#         gt[str(i)] = gt_rect
#     r = calc_deteval_metrics(pred,gt)
#     print(r)
